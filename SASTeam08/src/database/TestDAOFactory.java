...
public class TestDAOFactory extends DAOFactory implements IConnectionDriver {
	...
	private TestDAOFactory() {
		try {
			Document document = parseXML(new BufferedReader(new FileReader("WebContent/META-INF/context.xml")));
			...
			dataSource.setMaxTotal(3); // only allow three connections open at a time
